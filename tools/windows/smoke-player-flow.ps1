...
if ((Get-Content -LiteralPath $log -Raw) -notmatch 'PASS:') { Get-Content -LiteralPath $log -Tail 20; Get-Content (Join-Path $local 'player-flow-error.log') -Tail 12; throw 'Player-flow regression failed.' }
